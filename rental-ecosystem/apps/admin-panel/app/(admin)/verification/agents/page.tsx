...
function ActionMenu({ agent }: { agent: Agent }) {
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-48 rounded-lg border border-border bg-card shadow-lg z-40">
          <Link
            href={`/verification/review/${agent.id}`}
...
export default function AgentsVerificationPage() {
  const { agents } = generateAllMockData();
  const [selectedStatus, setSelectedStatus] = useState<string | null>(null);

  const filteredAgents = selectedStatus
    ? agents.filter((a) => a.verificationStatus === selectedStatus)
    : agents;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Manage and review real estate agent verification requests</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 rounded-lg border border-border bg-card p-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {verificationStatuses.map((status) => (
            ...
            >
              {status.label} ({filteredAgents.filter((a) => a.verificationStatus === status.value).length})
            </button>
          ))}
        </div>
      </div>
      ...
      <DataTable
        columns={columns}
        data={filteredAgents}
        actions={(agent) => <ActionMenu agent={agent} />}
      />
    </div>
  );
}
